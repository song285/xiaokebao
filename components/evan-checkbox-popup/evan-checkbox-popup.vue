				...
				<scroll-view scroll-y="true" class="evan-checkbox-popup__target__body">
					<evan-checkbox-group :max="max" v-model="currentValue">
						<view @click="toggleCheckbox(index)" class="evan-checkbox-popup__target__body__listitem" v-for="(item,index) in options"
						 :key="index">
							<text class="evan-checkbox-popup__target__body__listitem__label" :style="{color:isChecked(index)?primaryColor:'#333'}">{{item[optionLabel]}}</text>
							<evan-checkbox :primaryColor="primaryColor" ref="checkbox" :preventClick="true" :label="index">
								<template slot="icon">
									<view>
										<template v-if="isChecked(index)">
	...
	export default {
		...
		methods: {
			...
			toggleCheckbox(index) {
				this.$refs.checkbox[index].toggle(index)
			},
			onPopupChange(e) {
				// 捕捉uniPopup点击遮罩层关闭事件
				if (!e.show) {

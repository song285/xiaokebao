<template>
<uni-shadow-root class="vant-dist-tag-index"><view :class="'custom-class '+(utils.bem('tag', [type, size, { mark, plain, round }]))" :style="(color && !plain ? 'background-color: ' + color + ';' : '')+(textColor || (color && plain) ? 'color: ' + (textColor || color) : '')">
  <slot></slot>
